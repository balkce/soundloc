...
static const uint kNumberOfMoves = 7;
...
public:
	...
	/*
	 * @brief Randomly selects a near scan
	 * 
	 * @returns Selected near scan
	 */
	uint sampleNearScan(void) {          
	    std::default_random_engine generator;
	    std::uniform_int_distribution<int> distribution{0, max_miss_detections - 1};
  	    //std::discrete_distribution<int> distribution {10,10,3,3,3,2,2,1,1,1};
//	    std::discrete_distribution<int> distribution {10,7,5,3,2};
	...
	uint sampleMove(Partition& current)
	{
		...
		if (current.tracks.empty())
		    ...
		    std::uniform_int_distribution<int> distribution{0, kNumberOfMoves - 1};
	...
	double extendTrack(Track& track, Track& false_positives)
	{
		...
		while (prev_time < last_time && !end_track)
		{
		    ...
		    uint near_scan = sampleTime(0, max_miss_detections - 1);
		    ...
		    if (!candidate_pairs.empty()){
				...
				extend_probability *= (1.0 / (double) max_miss_detections) * (1.0 / (double) candidate_pairs.size());
	...
	{       
		...
		{
		    for (NeighborTime::iterator ob = nforest[birth][number][near_scan].begin(); 
				 ob != nforest[birth][number][near_scan].end(); ++ob){
		...
		if (!candidates_first_observation.empty()){		  
		    ...
		    track_probability *= (1.0 / (double)max_miss_detections) * (1.0 / (double) candidates_first_observation.size());
		    ...
		    if (!candidates_second_observation.empty()){
				...
				track_probability *= (1.0 / (double) max_miss_detections) * (1.0 / (double) candidates_second_observation.size());
	...
	Partition switchMove(Partition current)
	{
		// find pairs of tracks that can be switched
 	        std::vector< std::tuple<uint,uint,uint,uint> > switchable_tracks;
		for (uint i = 0; i < current.tracks.size() - 1; ++i){
		    ...
		    for (uint j = i + 1; j < current.tracks.size(); ++j){
				...
				for (uint p = 1; p < track_i->size() - 2; ++p){
					for (uint q = 1; q < track_j->size() - 2; ++q){
						if (track_i->at(p).time + max_miss_detections > track_j->at(q + 1).time && track_j->at(q).time + max_miss_detections > track_i->at(p + 1).time){
							Item item_i_p = track_i->at(p);
							Item item_i_p1 = track_i->at(p + 1);
							Item item_j_q = track_j->at(q);
							Item item_j_q1 = track_j->at(q + 1);
							bool found1 = false;

							for (int k = 0; k < max_miss_detections; ++k)
							{
								for (NeighborTime::iterator it = nforest[item_j_q.time][item_j_q.number][k].begin(); 
								     it != nforest[item_j_q.time][item_j_q.number][k].end(); ++it)
								{
							...
							for (int k = 0; k < max_miss_detections; ++k)
							{
								for (NeighborTime::iterator it = nforest[item_i_p.time][item_i_p.number][k].begin(); 
								     it != nforest[item_i_p.time][item_i_p.number][k].end(); ++it)
								{
							...
							if (found1 && found2)
							  switchable_tracks.push_back(std::make_tuple(i,j,p,q));
		...
		if (!switchable_tracks.empty()){
		    // randomly select a candidate pair to merge
   		    std::tuple<uint,uint,uint,uint> sample_switch_points = sampleCandidate(switchable_tracks);
		    TrackSet::iterator first_track = current.tracks.begin() + std::get<0>(sample_switch_points);
		    TrackSet::iterator second_track = current.tracks.begin() + std::get<1>(sample_switch_points);
	
		    Track switched_first(std::get<2>(sample_switch_points) + 1 + second_track->size() - (std::get<3>(sample_switch_points) + 1));
		    std::copy(first_track->begin(), first_track->begin() + std::get<2>(sample_switch_points) + 1, switched_first.begin());
		    std::copy(second_track->begin() + std::get<3>(sample_switch_points) + 1, second_track->end(), switched_first.begin() + std::get<2>(sample_switch_points) + 1);		    
		    std::copy(first_track->begin(), first_track->begin() + std::get<2>(sample_switch_points) + 1, switched_first.begin());

		    Track switched_second(std::get<3>(sample_switch_points) + 1 + first_track->size() - (std::get<2>(sample_switch_points) + 1));
		    std::copy(second_track->begin(), second_track->begin() + std::get<3>(sample_switch_points) + 1, switched_second.begin());
		    std::copy(first_track->begin(), first_track->begin() + std::get<2>(sample_switch_points) + 1, switched_first.begin());
		    std::copy(first_track->begin() + std::get<2>(sample_switch_points) + 1, first_track->end(), switched_second.begin() + std::get<3>(sample_switch_points) + 1);
		    std::copy(first_track->begin(), first_track->begin() + std::get<2>(sample_switch_points) + 1, switched_first.begin());
		    
		    // remove old tracks
		    current.tracks.erase(first_track);
		    current.tracks.erase(second_track);

		    // adds switched tracks
		    current.tracks.push_back(switched_first);
		    current.tracks.push_back(switched_second);
